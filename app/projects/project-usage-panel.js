    ...
    try {
      ...
      for (const url of LOCAL_USAGE_URLS) {
        try {
          ...
          if (!candidate.projects?.items && url !== LOCAL_USAGE_URLS[LOCAL_USAGE_URLS.length - 1]) {
            lastError = new Error("Local Token Meter does not include project usage yet");
            continue;
          }
          data = candidate;
          activeUrl = url;
      ...
      setError(
        usage
          ? "ローカル版に接続できないため、最後に取得したプロジェクト別使用量を表示しています。"
  ...
  if (!usage) {
    return (
      <section className="panel local-panel">
        ...
        <p className="muted">{error || "ローカル版 Token Meter からプロジェクト別データを取得します。"}</p>
      </section>
    );
  }
